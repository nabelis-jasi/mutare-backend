...
// Approve edit (engineer only)
router.put('/:edit_id/approve', auth, allowRoles('engineer'), async (req, res) => {
  const { edit_id } = req.params;
  const editRes = await pool.query('SELECT * FROM asset_edits WHERE id = $1 AND status = $2', [edit_id, 'pending']);
...
router.put('/:edit_id/reject', auth, allowRoles('engineer'), async (req, res) => {
  const { edit_id } = req.params;
  const result = await pool.query(
