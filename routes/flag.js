...
// Get all flags (engineer sees all, collector sees own)
...
// Resolve a flag (engineer only)
router.put('/:flag_id/resolve', auth, allowRoles('engineer'), async (req, res) => {
    const { flag_id } = req.params;
    await pool.query('UPDATE flags SET resolved = true, resolved_at = NOW(), resolved_by = $1 WHERE id = $2', [req.user.id, flag_id]);
