...
// Get all connections for the engineer
router.get('/', auth, allowRoles('engineer'), async (req, res) => {
    const result = await pool.query('SELECT id, name, pg_host, pg_port, pg_database, pg_user, geoserver_url, is_active FROM connections WHERE user_id = $1', [req.user.id]);
...
// Get active connection (returns geoserver_url for frontend)
router.get('/active', auth, allowRoles('engineer'), async (req, res) => {
    const result = await pool.query('SELECT geoserver_url, pg_host, pg_port, pg_database, pg_user FROM connections WHERE user_id = $1 AND is_active = true', [req.user.id]);
